...
objects.append(glh.Box(sbox2, (255, 0, 0))) # add a graphics object
...
control = box = pal.body.Box((-0,1.5,0,1,1,1), mass=75)
objects.append(glh.Box(box, (255, 0, 0)))

# all the balls
for x in xrange(-9,9,2):
    for y in xrange(-9,9,2):
        sphere = pal.body.Sphere((x,3,y,.8),10)
        objects.append(glh.Ball(sphere, (255, 0, 0)))

# set up links and actuators
revolute = pal.link.Revolute(sbox2, box, sbox2.get_position(),[0,1,0],False)

motor = pal.actuator.DCMotor(revolute,10,1,1)
motor.set_voltage(100)


running = True
gluLookAt(5,5,5,0,0,0,0,1,0) # change these numbers to move the camera arround
while running:
    motor.run() # make sure our actuator runs this step 
    pal.update(1./50.)
